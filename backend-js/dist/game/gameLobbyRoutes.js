...
function parseInviteToken(value) {
    ...
    if (extra.length || !/^[a-f0-9]{24}$/i.test(id !== null && id !== void 0 ? id : "") || !/^[a-f0-9]{64}$/i.test(secret !== null && secret !== void 0 ? secret : ""))
        return null;
    return { id, secret, token: `${id}.${secret}` };
}
function loadValidInvite(rawToken) {
    return __awaiter(this, void 0, void 0, function* () {
        const parsed = parseInviteToken(rawToken);
        if (!parsed)
            return null;
        const invite = yield __1.prisma.gameInviteLink.findUnique({ where: { id: parsed.id }, include: { game: true } });
...
function lobbyDto(game) {
    return __awaiter(this, void 0, void 0, function* () {
        const participants = yield __1.prisma.gameParticipant.findMany({
            where: { gameId: game.id },
            orderBy: { seat: "asc" },
            select: { gameUserId: true, seat: true },
        });
        const profiles = yield (0, gameIdentity_1.publicGameUsers)(participants.map((participant) => participant.gameUserId));
        const settings = (() => {
            var _a, _b;
            try {
...
function startLobbyInTransaction(tx_1, gameId_1) {
    return __awaiter(this, arguments, void 0, function* (tx, gameId, requireMinimum = true) {
        const game = yield tx.game.findUnique({ where: { id: gameId } });
        if (!game || game.status !== client_1.GameStatus.LOBBY)
            throw new Error("This lobby is no longer open");
        const participants = yield tx.gameParticipant.findMany({
            where: { gameId },
            orderBy: { seat: "asc" },
        });
        ...
        const now = new Date();
        yield tx.gameParticipant.updateMany({ where: { gameId }, data: { actionRequired: false } });
        yield tx.gameParticipant.update({
...
function createLinkLobbyRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        ...
        const type = (_b = (_a = req.body.game) !== null && _a !== void 0 ? _a : req.body.type) !== null && _b !== void 0 ? _b : "WORD_DROP";
        ...
        const game = yield __1.prisma.game.create({
            data: {
                type,
        ...
        return res.send((0, status_1.success)({ lobby: yield lobbyDto(game), inviteToken: `${linkId}.${linkSecret}` }));
    });
}
exports.createLinkLobbyRoute = createLinkLobbyRoute;
function listLobbiesRoute(_req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const gameUserId = res.locals.gameUserId;
        ...
        return res.send((0, status_1.success)(yield Promise.all(games.map(lobbyDto))));
    });
}
exports.listLobbiesRoute = listLobbiesRoute;
function getLobbyRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const gameId = Number(req.params.gameId);
        const gameUserId = res.locals.gameUserId;
        if (!Number.isInteger(gameId) || gameId <= 0)
            return res.send((0, status_1.clientError)("Invalid lobby"));
        const game = yield __1.prisma.game.findFirst({
        ...
        const invite = yield __1.prisma.gameInviteLink.findFirst({
            where: { gameId, revokedAt: null, expiresAt: { gt: new Date() } },
            select: { id: true },
        });
        return res.send((0, status_1.success)({ lobby: yield lobbyDto(game), gameId: game.id, status: game.status, hasActiveLink: Boolean(invite) }));
    });
}
exports.getLobbyRoute = getLobbyRoute;
function previewInviteRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const valid = yield loadValidInvite(req.params.token);
        if (!valid)
        ...
        return res.send((0, status_1.success)(yield lobbyDto(valid.invite.game)));
    });
}
exports.previewInviteRoute = previewInviteRoute;
function joinInviteRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const valid = yield loadValidInvite(req.params.token);
        if (!valid)
            return res.send((0, status_1.userError)("This game link is invalid or has expired"));
        ...
        let gameUser = suppliedSession ? yield (0, gameIdentity_1.resolveAnonymousSession)(suppliedSession) : null;
        let refreshToken = suppliedSession;
        if (!gameUser) {
            try {
                const created = yield (0, gameIdentity_1.createAnonymousGameUser)(typeof req.body.displayName === "string" ? req.body.displayName : "");
        ...
        try {
            const outcome = yield __1.prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                const game = yield tx.game.findUnique({ where: { id: valid.invite.gameId } });
                if (!game || game.status !== client_1.GameStatus.LOBBY)
                    throw new Error("This game has already started");
                const existing = yield tx.gameParticipant.findUnique({
                    where: { gameId_gameUserId: { gameId: game.id, gameUserId: gameUser.id } },
                });
                if (existing)
                    return { game, started: false };
                const participants = yield tx.gameParticipant.findMany({ where: { gameId: game.id }, select: { seat: true } });
                if (participants.length >= game.maxPlayers)
                ...
                while (occupied.has(seat))
                    seat += 1;
                yield tx.gameParticipant.create({ data: { gameId: game.id, gameUserId: gameUser.id, seat } });
                const joinedCount = participants.length + 1;
                if (joinedCount >= game.maxPlayers) {
                    return { game: yield startLobbyInTransaction(tx, game.id), started: true };
                }
                return {
                    game: yield tx.game.update({
                        where: { id: game.id },
            ...
            return res.send((0, status_1.success)({
                ...
                refreshToken,
                gameUser: yield (0, gameIdentity_1.publicGameUser)(gameUser.id),
                lobby: outcome.game.status === client_1.GameStatus.LOBBY ? yield lobbyDto(outcome.game) : null,
            }));
        }
        catch (error) {
...
function startLobbyRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const user = res.locals.user;
        if (!user)
        ...
        if (!Number.isInteger(gameId) || gameId <= 0)
            return res.send((0, status_1.clientError)("Invalid lobby"));
        const lobby = yield __1.prisma.game.findFirst({
        ...
        try {
            const game = yield __1.prisma.$transaction((tx) => startLobbyInTransaction(tx, gameId), { isolationLevel: client_1.Prisma.TransactionIsolationLevel.Serializable });
            (0, realtime_1.emitGameChanged)(game);
...
function cancelLobbyRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const user = res.locals.user;
        if (!user)
        ...
        if (!Number.isInteger(gameId) || gameId <= 0)
            return res.send((0, status_1.clientError)("Invalid lobby"));
        try {
            const cancelled = yield __1.prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                const result = yield tx.game.updateMany({
                    where: {
                        id: gameId,
                        status: client_1.GameStatus.LOBBY,
                        createdByAccountId: user.id,
                ...
                yield tx.gameInviteLink.updateMany({
...
function refreshAnonymousRoute(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const refreshToken = typeof req.body.refreshToken === "string" ? req.body.refreshToken : "";
        const gameUser = yield (0, gameIdentity_1.resolveAnonymousSession)(refreshToken);
        if (!gameUser)
