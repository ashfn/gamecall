...
export const EIGHT_BALL_V7_PHYSICS_VERSION = 7;
export const EIGHT_BALL_V7_STEP_HZ = 240;
export const EIGHT_BALL_V7_MAX_SECONDS = 20;
export const EIGHT_BALL_V7_MAX_SHOT_SPEED = 15_000;

const STEP_SECONDS = 1 / EIGHT_BALL_V7_STEP_HZ;
...
export interface EightBallV7FrameBall {
  number: number;
  x: number;
  y: number;
  vx: number;
...
export interface EightBallV7Frame {
  timeSeconds: number;
  balls: EightBallV7FrameBall[];
}

export interface EightBallV7Events {
...
export interface EightBallV7Profile {
  ticks: number;
...
export interface EightBallV7Options {
  captureHz?: number;
  maxSeconds?: number;
}

export interface EightBallV7Simulation {
  balls: EightBallPhysicsBall[];
  events: EightBallV7Events;
  durationSeconds: number;
  frames?: EightBallV7Frame[];
  profile: EightBallV7Profile;
}
...
function findCollisionBatch(
  ...
  for (let left = 0; left < state.count; left += 1) {
    if (state.pocketed[left]) continue;
    for (let right = left + 1; right < state.count; right += 1) {
      if (state.pocketed[right]) continue;
  ...
  for (let ball = 0; ball < state.count; ball += 1) {
    if (state.pocketed[ball]) continue;
...
function captureFrame(state: SolverState, timeSeconds: number): EightBallV7Frame {
  const balls: EightBallV7FrameBall[] = new Array(state.count);
  for (let index = 0; index < state.count; index += 1) {
    balls[index] = {
...
export function simulateEightBallShotV7(
  inputBalls: EightBallPhysicsBall[],
  shot: EightBallShotVector,
  options: EightBallV7Options = {},
): EightBallV7Simulation {
  ...
  const speed = power * EIGHT_BALL_V7_MAX_SHOT_SPEED / EIGHT_BALL_MAX_POWER;
  ...
  const events: EightBallV7Events = { firstHit: null, pocketed: [], cushionBalls: [], cueScratch: false };
  const cushionSet = new Set<number>();
  const profile: EightBallV7Profile = {
    ticks: 0,
    eventIterations: 0,
    eventLimitHits: 0,
    escapedBalls: 0,
  ...
  const maxSeconds = Math.max(STEP_SECONDS, Math.min(60, options.maxSeconds ?? EIGHT_BALL_V7_MAX_SECONDS));
  const maxTicks = Math.ceil(maxSeconds * EIGHT_BALL_V7_STEP_HZ);
  const captureHz = Math.max(0, Math.min(EIGHT_BALL_V7_STEP_HZ, Math.trunc(options.captureHz ?? 0)));
  const captureEveryTicks = captureHz ? Math.max(1, Math.round(EIGHT_BALL_V7_STEP_HZ / captureHz)) : 0;
...
export function eightBallV7MinimumSeparationSquared(balls: EightBallPhysicsBall[]): number {
...
export const EIGHT_BALL_V7_BALL_DIAMETER_SQUARED = BALL_DIAMETER_SQUARED;
