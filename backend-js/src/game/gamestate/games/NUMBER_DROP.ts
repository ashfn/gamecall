...
interface NumberDropMoveResult {
  state: NumberDropState;
  winner: number;
  nextPlayer: number;
}
...
export function normalizeNumberDropSettings(raw: unknown): NumberDropSettings {
  const candidate = raw && typeof raw === "object" ? raw as Partial<NumberDropSettings> : {};
...
function operationSymbol(operation: NumberDropOperation) {
...
function addSolvedValue(map: Map<number, ExpressionValue>, candidate: ExpressionValue) {
...
function buildNumberDropSolutions(numbers: number[]) {
  const size = 1 << numbers.length;
  const solved = Array.from({ length: size }, () => new Map<number, ExpressionValue>());
...
export function solveNumberDrop(numbers: number[], target: number) {
  const solved = buildNumberDropSolutions(numbers);
  let best: ExpressionValue | null = null;
  for (let mask = 1; mask < solved.length; mask += 1) {
  ...
  if (!best) throw new Error("This puzzle has no legal answers");
  return { ...best, distance: Math.abs(best.value - target) };
}

const hardTargetCache = new Map<string, Array<{ target: number; solution: ExpressionValue }>>();

function hardTargets(numbers: number[]) {
  const cacheKey = numbers.join(",");
  const cached = hardTargetCache.get(cacheKey);
  if (cached) return cached;

  const exactSolutions = new Map<number, ExpressionValue>();
  for (const values of buildNumberDropSolutions(numbers)) {
    for (const solution of values.values()) {
      if (solution.value < 101 || solution.value > 999) continue;
...
function puzzleForPlayers(player1: number, player2: number) {
  ...
  if (candidates.length === 0) throw new Error("Could not generate a Number Drop puzzle");
  const candidate = candidates[Math.floor(Math.random() * candidates.length)];
  return { numbers, ...candidate };
}

export function createNumberDropState(player1: number, player2: number, rawSettings: unknown = {}): NumberDropState {
...
export function normalizeNumberDropState(raw: unknown): NumberDropState {
  if (!raw || typeof raw !== "object") throw new Error("Invalid Number Drop state");
  const candidate = raw as NumberDropState;
  const state: NumberDropState = {
    ...candidate,
  ...
  if (state.kind !== "number-drop" || !Number.isInteger(state.player1) || !Number.isInteger(state.player2)) throw new Error("Invalid Number Drop players");
  if (!Array.isArray(state.numbers) || state.numbers.length !== 6 || state.numbers.some((number) => !Number.isSafeInteger(number) || number <= 0)) throw new Error("Invalid Number Drop numbers");
  if (!Number.isSafeInteger(state.target) || state.target < 100 || state.target > 999) throw new Error("Invalid Number Drop target");
  return state;
}

export function viewNumberDropState(raw: unknown, viewerId: number): NumberDropState {
...
export function evaluateNumberDropSubmission(numbers: number[], steps: NumberDropStep[], resultId: string) {
  if (!Array.isArray(steps) || steps.length > numbers.length - 1) throw new Error("Invalid answer route");
  const available = new Map<string, EvaluatedTile>(numbers.map((value, index) => [`n${index}`, { value, expression: String(value) }]));
  const normalizedSteps: NumberDropStep[] = [];
  steps.forEach((step, index) => {
    if (!step || typeof step.leftId !== "string" || typeof step.rightId !== "string" || step.leftId === step.rightId) throw new Error("Invalid answer route");
    if (!["ADD", "SUBTRACT", "MULTIPLY", "DIVIDE"].includes(step.operation)) throw new Error("Invalid operation");
    const left = available.get(step.leftId);
    const right = available.get(step.rightId);
    if (!left || !right) throw new Error("A number was reused or is unavailable");
    const value = calculateNumberDropOperation(left.value, right.value, step.operation);
    if (value === null) throw new Error("That calculation is not allowed");
  ...
  if (!result) throw new Error("Choose an available result to submit");
  return { ...result, steps: normalizedSteps };
}

function applyNumberDropSubmission(state: NumberDropState, playerId: number, submission: NumberDropSubmission): NumberDropMoveResult {
  const submissions: Record<string, NumberDropSubmission | null> = { ...state.submissions, [playerId]: submission };
  ...
  if (!opponentSubmission) return { state: { ...state, submissions }, winner: 0, nextPlayer: opponentId };
  ...
  const roundResult: NumberDropRoundResult = {
...
function assertCanSubmit(state: NumberDropState, playerId: number) {
  if (playerId !== state.player1 && playerId !== state.player2) throw new Error("You are not in this game");
  if (state.submissions[String(playerId)]) throw new Error("You already submitted an answer");
}

export function applyNumberDropMove(rawState: unknown, playerId: number, rawMove: unknown) {
  const state = normalizeNumberDropState(rawState);
  assertCanSubmit(state, playerId);
  if (!rawMove || typeof rawMove !== "object") throw new Error("Submit an answer");
  const move = rawMove as { kind?: unknown; steps?: unknown; resultId?: unknown };
  if (move.kind !== "submit" || !Array.isArray(move.steps) || typeof move.resultId !== "string") throw new Error("Submit a valid answer");
  const evaluated = evaluateNumberDropSubmission(state.numbers, move.steps as NumberDropStep[], move.resultId);
...
export function applyNumberDropTurnTimeout(rawState: unknown, playerId: number) {
  const state = normalizeNumberDropState(rawState);
  assertCanSubmit(state, playerId);
  return applyNumberDropSubmission(state, playerId, {
    playerId,
