...
import {
  EIGHT_BALL_V7_BALL_DIAMETER_SQUARED,
  eightBallV7MinimumSeparationSquared,
...
test("v7 resolves an identical quantized break every time", () => {
  const shot = { aimX: 0, aimY: -10000, power: 900 };
  const first = simulateEightBallShotV7(createEightBallRack(), shot);
  const second = simulateEightBallShotV7(createEightBallRack(), shot);
  ...
  assert.ok(
    eightBallV7MinimumSeparationSquared(first.balls) >= EIGHT_BALL_V7_BALL_DIAMETER_SQUARED,
    "settled balls must not overlap",
  );
});

test("v7 swept contacts stop a full-power shot tunnelling through a ball", () => {
  ...
  const simulation = simulateEightBallShotV7(
    balls,
  ...
  assert.ok(
    eightBallV7MinimumSeparationSquared(simulation.balls) >= EIGHT_BALL_V7_BALL_DIAMETER_SQUARED,
    "the colliding pair must finish separated",
  );
});

test("v7 rolling resistance preserves travel direction before contact", () => {
...
test("v7 requires a clean side-pocket approach", () => {
  const shootFrom = (y: number) => {
    const balls = tableWithOnly([0]);
    Object.assign(ballByNumber(balls, 0), { x: 500, y, pocketed: false });
    return simulateEightBallShotV7(balls, { aimX: -10000, aimY: 0, power: 80 });
  };
...
test("v7 is horizontally mirror-symmetric", () => {
  const shotFrom = (x: number, aimX: number) => {
    const balls = tableWithOnly([0]);
    Object.assign(ballByNumber(balls, 0), { x, y: 2400, pocketed: false });
    return simulateEightBallShotV7(balls, { aimX, aimY: -4000, power: 350 });
  };
...
test("v7 broad phase rejects most irrelevant pair quadratics", () => {
  const simulation = simulateEightBallShotV7(
    createEightBallRack(),
...
test("v7 never increases translational energy between contact-free captured frames", () => {
  const balls = tableWithOnly([0]);
...
test("v7 uses the same physical ball diameter as the production table", () => {
  assert.equal(EIGHT_BALL_V7_BALL_DIAMETER_SQUARED, (EIGHT_BALL_BALL_RADIUS * 2) ** 2);
});

test("v7 contains and separates a deterministic spread of arbitrary shots", () => {
  let seed = 0x51f15e;
  const random = () => {
  ...
  for (let shotIndex = 0; shotIndex < 128; shotIndex += 1) {
    ...
    const simulation = simulateEightBallShotV7(createEightBallRack(), {
