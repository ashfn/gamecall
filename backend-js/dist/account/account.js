...
function createJwt(user) {
    ...
    return jsonwebtoken_1.default.sign({
        "id": user.id
    }, secret, { expiresIn: (process.env.JWT_ACCESS_TTL ?? "2h") });
}
...
async function createRefreshSession(account) {
...
async function upgradeLegacyRefreshToken(value) {
    ...
    if (!user || !(await bcrypt_1.default.compare(parts[0], user.refreshToken)))
        return null;
...
async function refreshToken(refreshToken) {
    ...
    if (sessionToken) {
        const now = new Date();
        const session = await __1.prisma.refreshSession.findUnique({
        ...
        if (!session || session.expiresAt <= now || !validHash) {
            if (session?.expiresAt && session.expiresAt <= now) {
                await __1.prisma.refreshSession.delete({ where: { id: session.id } }).catch(() => undefined);
            }
            return (0, status_1.clientError)("Invalid refresh token");
        }
        await __1.prisma.$transaction([
    ...
    const user = await __1.prisma.user.findFirst({
        where: {
            id: userId
        }
    });
    ...
    const match = await bcrypt_1.default.compare(token, user.refreshToken);
    if (!match) {
        return (0, status_1.clientError)("Invalid refresh token");
    }
...
async function register(username, email, password) {
    ...
    const checkIdentical = await __1.prisma.user.findFirst({
        where: {
            OR: [
                { username: {
                        equals: username,
    ...
    await __1.prisma.$transaction(async (tx) => {
        ...
        await (0, gameIdentity_1.ensureAccountGameUser)(user.id, tx);
    });
...
async function login(usernameOrEmail, password) {
    ...
    const account = await __1.prisma.user.findFirst({
        where: {
            OR: [
                { username: {
    ...
    await (0, gameIdentity_1.ensureAccountGameUser)(account.id);
...
async function logout(userId, refreshToken) {
    ...
    if (sessionToken) {
        await __1.prisma.refreshSession.deleteMany({ where: { id: sessionToken.id, userId } });
    }
    else {
        // Legacy clients did not send their refresh token on logout.
        await __1.prisma.user.update({ where: { id: userId }, data: { refreshToken: "none" } });
    }
    return (0, status_1.success)();
}
