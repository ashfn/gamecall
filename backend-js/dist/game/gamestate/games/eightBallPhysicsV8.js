...
exports.EIGHT_BALL_V8_BALL_DIAMETER_SQUARED = exports.eightBallV8MinimumSeparationSquared = exports.simulateEightBallShotV8 = exports.EIGHT_BALL_V8_MAX_SHOT_SPEED = exports.EIGHT_BALL_V8_MAX_SECONDS = exports.EIGHT_BALL_V8_STEP_HZ = exports.EIGHT_BALL_V8_PHYSICS_VERSION = void 0;
 ...
 */
exports.EIGHT_BALL_V8_PHYSICS_VERSION = 8;
exports.EIGHT_BALL_V8_STEP_HZ = 240;
exports.EIGHT_BALL_V8_MAX_SECONDS = 20;
exports.EIGHT_BALL_V8_MAX_SHOT_SPEED = 16200;
const STEP_SECONDS = 1 / exports.EIGHT_BALL_V8_STEP_HZ;
...
const POCKET_DROP_ACCELERATION = 18000;
...
function findCollisionBatch(state, maxTime, batch, profile, authoritativePocketFall) {
    resetBatch(batch);
    for (let left = 0; left < state.count; left += 1) {
        if (state.pocketed[left] || state.sinkingPocket[left] >= 0)
            continue;
        for (let right = left + 1; right < state.count; right += 1) {
            if (state.pocketed[right] || state.sinkingPocket[right] >= 0)
                continue;
    ...
    for (let ball = 0; ball < state.count; ball += 1) {
        ...
        for (let pocketIndex = 0; pocketIndex < eightBallPhysics_1.EIGHT_BALL_POCKETS.length; pocketIndex += 1) {
            ...
            if (authoritativePocketFall) {
                const inwardLength = Math.hypot(eightBallPhysics_1.EIGHT_BALL_TABLE_WIDTH / 2 - pocket.x, eightBallPhysics_1.EIGHT_BALL_TABLE_HEIGHT / 2 - pocket.y) || 1;
                const inwardX = (eightBallPhysics_1.EIGHT_BALL_TABLE_WIDTH / 2 - pocket.x) / inwardLength;
                const inwardY = (eightBallPhysics_1.EIGHT_BALL_TABLE_HEIGHT / 2 - pocket.y) / inwardLength;
                if (velocityX * inwardX + velocityY * inwardY >= -TIME_EPSILON)
                    continue;
            }
...
function advance(state, seconds) {
    ...
    for (let index = 0; index < state.count; index += 1) {
        ...
        const rearWall = -eightBallPhysics_1.EIGHT_BALL_BALL_RADIUS;
        if (!state.pocketLinerHit[index]) {
            ...
            if (forward <= rearWall || (timeToRear >= 0 && timeToRear <= remaining)) {
                ...
                if (forwardVelocity < 0)
                    forwardVelocity *= -POCKET_LINER_RESTITUTION;
        ...
        if (Math.abs(lateral) > sideWall) {
            ...
            if (lateralVelocity * side > 0)
                lateralVelocity *= -POCKET_SIDE_RESTITUTION;
        }
...
function resolveBatch(state, batch, events, cushionSet, profile, preservePocketEntry) {
    let resolved = 0;
...
function finalizePocketFalls(state, events) {
    for (let ball = 0; ball < state.count; ball += 1) {
        if (state.pocketed[ball] || state.sinkingPocket[ball] < 0
...
function applyRollingResistance(state) {
    ...
    for (let index = 0; index < state.count; index += 1) {
        if (state.pocketed[index] || state.sinkingPocket[index] >= 0)
            continue;
...
function simulateEightBallShotV8(inputBalls, shot, options = {}) {
    ...
    const speed = power * exports.EIGHT_BALL_V8_MAX_SHOT_SPEED / eightBallPhysics_1.EIGHT_BALL_MAX_POWER;
    ...
    for (let tick = 1; tick <= maxTicks; tick += 1) {
        ...
        if (remaining > TIME_EPSILON)
            advance(state, remaining);
        if (options.authoritativePocketFall === true)
            finalizePocketFalls(state, events);
...
exports.simulateEightBallShotV8 = simulateEightBallShotV8;
function eightBallV8MinimumSeparationSquared(balls) {
...
exports.eightBallV8MinimumSeparationSquared = eightBallV8MinimumSeparationSquared;
exports.EIGHT_BALL_V8_BALL_DIAMETER_SQUARED = BALL_DIAMETER_SQUARED;
